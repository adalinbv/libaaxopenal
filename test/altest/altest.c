...
#define FILENAME_BOOM      SRC_PATH"sounds/boom.ogg"
#define FILENAME_DING      SRC_PATH"sounds/ding.wav"
#define FILENAME_FOOTADPCM SRC_PATH"sounds/footadpcm.wav"
#define FILENAME_FOOTSTEPS SRC_PATH"sounds/footsteps.wav"
#define FILENAME_STEREO    SRC_PATH"sounds/stereo.wav"
#define FILENAME_WAVE1     SRC_PATH"sounds/wave1.wav"
#define FILENAME_WAVE2     SRC_PATH"sounds/wave2.wav"
#define FILENAME_WAVE3     SRC_PATH"sounds/wave3.wav"
#define FILENAME_WAVE4     SRC_PATH"sounds/wave4.wav"
...
#include <conio.h>
# define KBHIT() _kbhit()
#elif HAVE_IOCTL && HAVE_TERMIOS_H
int ioctl_kbhit(void);
# define KBHIT() ioctl_kbhit()
#else
# define KBHIT() 1
# define NO_KB_HIT_FN
#endif
...
/* Function prototypes */
ALvoid DisplayALError (const ALchar *szText, ALint errorcode);
...
*/
ALvoid
DisplayALError (const ALchar *szText, ALint errorcode)
{
...
ALvoid
getWAVData (const char *file, ALenum *format, ALvoid **data, ALsizei *size,
              ...
              if (!memcmp (ChunkHdr.Id, "fmt ", 4))
                {
                  ...
                  if ((FmtHdr.Format == 0x0001) || (FmtHdr.Format == 0xFFFE))
                      ...
                      if (FmtHdr.Channels == 1)
                        *format =
                          (FmtHdr.BitsPerSample ==
                           4 ? alGetEnumValue (ALCHAR"AL_FORMAT_MONO_IMA4")
                           : (FmtHdr.BitsPerSample ==
                              8 ? AL_FORMAT_MONO8 : AL_FORMAT_MONO16));
                      else if (FmtHdr.Channels == 2)
                        *format =
                          (FmtHdr.BitsPerSample ==
                           4 ? alGetEnumValue (ALCHAR"AL_FORMAT_STEREO_IMA4")
                           : (FmtHdr.BitsPerSample ==
                    ...
                    {
                      *format = alGetEnumValue (ALCHAR"AL_FORMAT_MP3");
                      *freq = FmtHdr.SamplesPerSec;
                      fseek (Stream,
                             ChunkHdr.Size - sizeof (WAVFmtHdr_Struct),
                             SEEK_CUR);
...
LoadWave (char *szWaveFile, ALuint BufferID)
{
  ...
  if ((error = alGetError ()) != AL_NO_ERROR)
    {
      DisplayALError (ALCHAR"alBufferData : ", error);
      return AL_FALSE;
    }
...
#if defined(HAVE_IOCTL) && defined(HAVE_TERMIOS_H)
# include <sys/ioctl.h>
# include <termios.h>
int ioctl_kbhit(void)
{
...
main (int argc, char *argv[])
{
  ...
  if (strlen (deviceName) == 0)
    ...
    {
      Device = alcOpenDevice ((ALCchar *) deviceName);   /* have a name from enumeration process above, so use it... */
    }
  /*Create context(s) */
  ...
  g_bOffsetExt = alIsExtensionPresent (ALCHAR"al_ext_offset");
  if (g_bOffsetExt)
    printf ("AL_EXT_OFFSET support found !\n");

  g_bNewDistModels &= alIsExtensionPresent (ALCHAR"AL_EXT_LINEAR_DISTANCE");
  g_bNewDistModels &= alIsExtensionPresent (ALCHAR"AL_EXT_EXPONENT_DISTANCE");
  if (g_bNewDistModels)
    printf
          ...
          if (stat (FILENAME_BOOM, &sbuf) != -1)
            {
              ...
              if (ovdata != NULL)
                {
                  ALenum formatVorbis =
                    alGetEnumValue (ALCHAR"AL_FORMAT_VORBIS_EXT");
                  ...
                  if (error != AL_NO_ERROR)
                    {
                      DisplayALError (ALCHAR"alBufferData buffer 7 : ",
                                      error);
                    }
                  free (ovdata);
                }
  ...
  for (i = 0; enumerationALC[i].enumName; i++)
    {
      ...
      alGetError ();
      actualValueAL = alGetEnumValue ((const ALchar*)enumName);
      if (actualValueAL != 0)
        {
          printf
  ...
  for (i = 0; enumerationAL[i].enumName; i++)
    {
      ...
      /* Check that retrieving an AL enum via alcGetEnumValue yields an
         error. */
      alcGetError (device);
      actualValueALC = alcGetEnumValue (device, (ALCchar *)enumName);
  ...
  if (ContinueOrSkip ())
    {
      ...
      tempString = (ALchar *)alcGetString (pDevice, ALC_DEVICE_SPECIFIER);
...
I_QueueTest (ALvoid)
{
  ...
  if ((error = alGetError ()) != AL_NO_ERROR)
    {
      DisplayALError (ALCHAR"alGenSources 1 : ", error);
      return;
    }

  alSourcef (source[0], AL_PITCH, 1.0f);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcef 0 AL_PITCH : ", error);

  alSourcef (source[0], AL_GAIN, 1.0f);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcef 0 AL_GAIN : ", error);

  alSourcefv (source[0], AL_POSITION, source0Pos);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcefv 0 AL_POSITION : ", error);

  alSourcefv (source[0], AL_VELOCITY, source0Vel);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcefv 0 AL_VELOCITY : ", error);

  alSourcei (source[0], AL_LOOPING, AL_FALSE);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 0 AL_LOOPING AL_FALSE: ", error);
  ...
  do
    {
      ...
      switch (ch)
        ...
        case '1':
          alSourcePlay (source[0]);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcePlay source 0 : ", error);
          break;
        case '2':
          alSourceStop (source[0]);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourceStop source 0 : ", error);
          break;
        case '3':
          if (bLooping == AL_TRUE)
            {
          ...
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcei AL_LOOPING : ", error);
          break;
        case '4':
          alSourceQueueBuffers (source[0], 4, buffers);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourceQueueBuffers 4 : ", error);
          break;
        case '5':
          alSourceQueueBuffers (source[0], 1, &buffers[0]);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourceQueueBuffers 1 : ", error);
          break;
        case '6':
        ...
        case '9':
          ...
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourceQueueBuffers 1 (buffer 0) : ", error);
          break;
        case 'A':
          {
            ...
            if ((error = alGetError ()) != AL_NO_ERROR)
              {
                DisplayALError (ALCHAR"alSourceUnqueueBuffers 1 : ", error);
              }
            else
              {
        ...
        case 'B':
          {
            ...
            if ((error = alGetError ()) != AL_NO_ERROR)
              {
                DisplayALError (ALCHAR"alSourceUnqueueBuffers 2 : ", error);
              }
            else
              {
                for (i = 0; i < 2; i++)
        ...
        case 'C':
          {
            ...
            if ((error = alGetError ()) != AL_NO_ERROR)
              {
                DisplayALError (ALCHAR"alSourceUnqueueBuffers 3 : ", error);
              }
            else
              {
                for (i = 0; i < 3; i++)
        ...
        case 'F':
          ...
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSource AL_BUFFER NULL : ", error);
          break;
        case '0':
  ...
  alSourceStop (source[0]);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourceStop : ", error);

  alDeleteSources (1, source);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alDeleteSources 1 : ", error);
}
...
I_FreqTest (ALvoid)
{
  ...
  alSourcef (source[0], AL_PITCH, 1.0f);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcef 0 AL_PITCH : \n", error);

  alSourcef (source[0], AL_GAIN, 1.0f);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcef 0 AL_GAIN : \n", error);

  alSourcefv (source[0], AL_POSITION, source0Pos);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcefv 0 AL_POSITION : \n", error);

  alSourcefv (source[0], AL_VELOCITY, source0Vel);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcefv 0 AL_VELOCITY : \n", error);

  alSourcei (source[0], AL_BUFFER, g_Buffers[1]);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 0 AL_BUFFER buffer 1 : \n", error);

  alSourcei (source[0], AL_LOOPING, AL_TRUE);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 0 AL_LOOPING AL_TRUE: \n", error);
  ...
  do
    {
      ...
      switch (ch)
        ...
        case '2':
          ...
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcef source 0 AL_PITCH 2.0 : ",
                            error);
          break;
        case '3':
          alSourcef (source[0], AL_PITCH, 0.5f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcef source 0 AL PITCH 0.5: ",
                            error);
          break;
        }
    }
  ...
  alSourceStopv (1, source);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourceStopv 2 : ", error);

  alDeleteSources (1, source);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alDeleteSources 2 : ", error);
}
  ...
  do
    {
      ...
      switch (ch)
        ...
        case '2':
          ...
          alSourcei (source[0], AL_LOOPING, bLoop);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcei 0 AL_LOOPING : \n", error);
        ...
        case '4':
          ...
          alSourcei (source[0], AL_BUFFER, 0);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcei AL_BUFFER (NULL) : ",
                            error);

          alSourceQueueBuffers (source[0], 2, buffers);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourceQueueBuffers 2 (stereo) : ",
                            error);
        ...
        case '5':
          ...
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourceUnqueueBuffers 2 (stereo) : ",
                            error);
          break;
        case '6':
          if (bLoop)
...
I_GainTest (ALvoid)
{
  ...
  if ((error = alGetError ()) != AL_NO_ERROR)
    {
      DisplayALError (ALCHAR"alGenSources 2 : ", error);
      return;
    }
  ...
  alSourcei (source[0], AL_BUFFER, g_Buffers[0]);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 0 AL_BUFFER buffer 0 : \n", error);
  ...
  alSourcef (source[1], AL_PITCH, 1.0f);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcef 1 AL_PITCH : \n", error);

  alSourcef (source[1], AL_GAIN, 1.0f);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcef 1 AL_GAIN : \n", error);

  alSourcefv (source[1], AL_POSITION, source1Pos);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcefv 1 AL_POSITION : \n", error);

  alSourcefv (source[1], AL_VELOCITY, source1Vel);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcefv 1 AL_VELOCITY : \n", error);

  alSourcei (source[1], AL_BUFFER, g_Buffers[1]);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 1 AL_BUFFER buffer 1 : \n", error);

  alSourcei (source[1], AL_LOOPING, AL_TRUE);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 1 AL_LOOPING AL_TRUE: \n", error);
  ...
  do
    {
      ...
      switch (ch)
        ...
        case '2':
          alSourcePlay (source[1]);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcePlay source 1 : ", error);
          break;
        case '3':
          alSourceStop (source[0]);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourceStop source 0 : \n", error);
          break;
        case '4':
          alSourceStop (source[1]);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourceStop source 1 : \n", error);
          break;
        case '5':
          alSourcef (source[0], AL_GAIN, 1.0f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcef 0 AL_GAIN 1.0 : \n", error);
          break;
        case '6':
          alSourcef (source[0], AL_GAIN, 0.5f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcef 0 AL_GAIN 0.5 : \n", error);
          break;
        case '7':
          alSourcef (source[0], AL_GAIN, 0.25f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcef 0 AL_GAIN 0.25 : \n",
                            error);
          break;
        case '8':
          alSourcef (source[0], AL_GAIN, 0.0f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcef 0 AL_GAIN 0.0 : \n", error);
          break;
        case 'A':
          alListenerf (AL_GAIN, 1.0f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alListenerf AL_GAIN 1.0 : \n", error);
          break;
        case 'B':
          alListenerf (AL_GAIN, 0.5f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alListenerf AL_GAIN 0.5 : \n", error);
          break;
        case 'C':
          alListenerf (AL_GAIN, 0.25f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alListenerf AL_GAIN 0.25 : \n",
                            error);
          break;
        case 'D':
          alListenerf (AL_GAIN, 0.0f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alListenerf AL_GAIN 0.0 : \n", error);
          break;
        }
    }
  while (ch != 'Q');
...
I_StreamingTest (ALvoid)
{
  ...
  if ((error = alGetError ()) != AL_NO_ERROR)
    {
      DisplayALError (ALCHAR"alGenBuffers : ", error);
      fclose (fp);
      return;
    }
  ...
  do
    {
      ...
      switch (ch)
          ...
          for (i = 0; i < numSources; i++)
            {
              ...
              if ((error = alGetError ()) != AL_NO_ERROR)
                DisplayALError (ALCHAR"alSourcePlay : ", error);
        ...
        case '2':
          ...
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourceStopv : ", error);
          break;
        }
    }
  while (ch != 'Q');
...
I_SourceRelativeTest ()
{
  ...
  alSourcei (source[0], AL_BUFFER, g_Buffers[1]);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 0 AL_BUFFER buffer 0 : ", error);

  alSourcei (source[0], AL_LOOPING, AL_TRUE);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 0 AL_LOOPING AL_TRUE: ", error);

  alSourcei (source[0], AL_SOURCE_RELATIVE, AL_FALSE);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 0 AL_SOURCE_RELATIVE AL_FALSE : ", error);
  ...
  do
    {
      ...
      switch (ch)
        ...
        case '2':
          ...
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alListenerfv  AL_POSITION : ", error);
          break;
        case '3':
          if (bRelative)
            {
          ...
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcei source 0 AL_SOURCE_RELATIVE : ",
                            error);
          break;
        }
    }
  ...
  alSourceStopv (1, source);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourceStopv 1 : ", error);
...
I_SourceRolloffTest ()
{
  ...
  alSourcefv (source[1], AL_POSITION, source1Pos);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcefv 1 AL_POSITION : ", error);

  alSourcei (source[1], AL_BUFFER, g_Buffers[1]);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 1 AL_BUFFER buffer 1 : ", error);

  alSourcei (source[1], AL_LOOPING, AL_TRUE);
  if ((error = alGetError ()) != AL_NO_ERROR)
    DisplayALError (ALCHAR"alSourcei 1 AL_LOOPING AL_FALSE: ", error);
  ...
  do
    {
      ...
      switch (ch)
        ...
        case '4':
          alSourceStop (source[1]);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourceStop source 1 : ", error);
          break;
        case '5':
          alSourcef (source[0], AL_ROLLOFF_FACTOR, 0.5f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcef ROLLOFF_FACTOR 0.5 : ", error);
          break;
        case '6':
          alSourcef (source[0], AL_ROLLOFF_FACTOR, 1.0f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcef ROLLOFF_FACTOR 1.0 : ", error);
          break;
        case '7':
          alSourcef (source[0], AL_ROLLOFF_FACTOR, 2.0f);
          if ((error = alGetError ()) != AL_NO_ERROR)
            DisplayALError (ALCHAR"alSourcef ROLLOFF_FACTOR 2.0 : ", error);
          break;
        case '8':
...
I_ADPCMTest ()
{
  ...
  eMonoADPCM = alGetEnumValue (ALCHAR"AL_FORMAT_MONO_IMA4");
  eStereoADPCM = alGetEnumValue (ALCHAR"AL_FORMAT_STEREO_IMA4");
...
#ifdef NO_KB_HIT_FN
  printf("\nWarning:\n");
  printf("   Your system doesn't have a way to detect a keyboard hit.\n");
  printf("   This means tracking progress isn't functional.\n\n");
#endif
      ...
      else
        {
          switch (iType)
            ...
            case 0:
              alGetSourcei (Source, AL_BYTE_OFFSET, &lOffset);
              if ((error = alGetError ()) != AL_NO_ERROR)
                DisplayALError (ALCHAR"alGetSourcei AL_BYTE_OFFSET : ", error);
              else
                printf ("ByteOffset is %d                  \r", lOffset);
              break;
            ...
            case 1:
              alGetSourcei (Source, AL_SAMPLE_OFFSET, &lOffset);
              if ((error = alGetError ()) != AL_NO_ERROR)
                DisplayALError (ALCHAR"alGetSourcei AL_SAMPLE_OFFSET : ", error);
              else
                printf ("SampleOffset is %d                \r", lOffset);
              break;
            ...
            case 2:
              alGetSourcef (Source, AL_SEC_OFFSET, &flOffset);
              if ((error = alGetError ()) != AL_NO_ERROR)
                DisplayALError (ALCHAR"alGetSourcef AL_SEC_OFFSET : ", error);
              else
  ...
  if (alIsExtensionPresent ((ALchar *) "AL_EXT_vorbis") == AL_TRUE)
    {
      ...
      if ((error = alGetError ()) != AL_NO_ERROR)
        {
          DisplayALError (ALCHAR"alGenSources 3 : ", error);
          return;
        }
            ...
            case '4':
              {
                ...
                if ((error = alGetError ()) != AL_NO_ERROR)
                  DisplayALError (ALCHAR"alSourceStop source 2 : ",
                                  error);
                ...
                if (data != 0)
                  {
                    ...
                    if ((error = alGetError ()) != AL_NO_ERROR)
                      DisplayALError (ALCHAR"alSourcef 2 AL_GAIN : \n",
                                      error);

                    /* Play source */
                    alSourcePlay (source[2]);
                    if ((error = alGetError ()) != AL_NO_ERROR)
                      DisplayALError (ALCHAR"alSourcePlay source 2 : ",
                                      error);
      ...
      if ((error = alGetError ()) != AL_NO_ERROR)
        DisplayALError (ALCHAR"alDeleteSources 3 : ", error);
    }
  else
